...
@Slf4j
class GcpDummyCloudProvider implements CloudProvider {
	public static final String CLOUD_PROVIDER_CODE = 'GcpDummy.cloud'

	protected MorpheusContext context
//	protected Plugin plugin
	GcpDummyPlugin plugin

	public GcpDummyCloudProvider(GcpDummyPlugin plugin, MorpheusContext ctx) {
		super()
		this.@plugin = plugin
		this.@context = ctx
	}
	...
	Collection<ComputeServerType> getComputeServerTypes() {
		def serverTypes = [
				new ComputeServerType(code: 'googleUnmanaged', name: 'Google Instance', description: 'google vm', platform: 'linux', agentType: ComputeServerType.AgentType.guest,
						enabled: true, selectable: false, externalDelete: true, managed: false, controlPower: true, controlSuspend: false, creatable: false, computeService: 'googleComputeService',
						displayOrder: 99, hasAutomation: false, provisionTypeCode: 'google',
						containerHypervisor: false, bareMetalHost: false, vmHypervisor: false, managedServerType: 'googleVm', guestVm: true, supportsConsoleKeymap: true,
						optionTypes: []
				)
		]
		return serverTypes
	...
	ServiceResponse refresh(Cloud cloudInfo) {
		// syncing VMs
		log.info("Refreshing VMs for cloud: ${cloudInfo}")
		new VmSync(plugin,cloudInfo).execute()
		return ServiceResponse.success()
	}
	...
	ServiceResponse startServer(ComputeServer computeServer) {
		log.info("Power on for this VM ${computeServer.name}")
		def projectId = computeServer.getConfigProperty('projectId').toString()
		def googleZoneId = computeServer.getConfigProperty('googleZoneId').toString()
		def instanceId = computeServer.externalId
		log.info("Power on for this VM ${computeServer.name} ${projectId} ${googleZoneId} ${instanceId}")
		GcpClient client = new GcpClient(plugin,computeServer.cloud)
		client.PowerOnInstance(projectId,instanceId,googleZoneId)
		return ServiceResponse.success()
	}
	...
	ServiceResponse stopServer(ComputeServer computeServer) {
		log.info("Power off for this VM ${computeServer.name}")
		GcpClient client = new GcpClient(plugin,computeServer.cloud)
		def projectId = computeServer.getConfigProperty('projectId').toString()
		def googleZoneId = computeServer.getConfigProperty('googleZoneId').toString()
		def instanceId = computeServer.externalId
		log.info("Power off for this VM ${computeServer.name} ${projectId} ${googleZoneId} ${instanceId}")
		client.PowerOffInstance(projectId,instanceId,googleZoneId)
		return ServiceResponse.success()
	}
	...
	ServiceResponse deleteServer(ComputeServer computeServer) {
		log.info("Deleting this VM: ${computeServer.name}")
		def projectId = computeServer.getConfigProperty('projectId').toString()
		def googleZoneId = computeServer.getConfigProperty('googleZoneId').toString()
		def instanceId = computeServer.externalId
		log.info("Deleting this VM: ${computeServer.name} ${projectId} ${googleZoneId} ${instanceId}")
		GcpClient client = new GcpClient(plugin,computeServer.cloud)
		client.deleteInstance(projectId,instanceId,googleZoneId)
		return ServiceResponse.success()
	}
	...
	String getDefaultProvisionTypeCode() {
		return GcpDummyProvisionProvider.PROVISION_PROVIDER_CODE
	}
